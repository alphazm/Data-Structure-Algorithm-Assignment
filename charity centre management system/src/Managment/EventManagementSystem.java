...
public class EventManagementSystem {
        ...
        volunteer volunteer2 = new volunteer("V002", "Jane", "Smith", "jane@example.com", 
                                             "098-765-4321", "456 Elm St", "Helper", true);
        volunteer volunteer3 = new volunteer("V003", "Wei long", "Heng", "hwl@example.com", 
                                             "098-765-4321", "456 Elm St", "Coordinator", true);
        volunteer volunteer4 = new volunteer("V004", "Alden", "Ling", "Alden@example.com", 
                                             "098-765-4321", "456 Elm St", "Helper", true);
        volunteer volunteer5 = new volunteer("V005", "Harry", "Chiong", "harry@example.com", 
                                             "098-765-4321", "456 Elm St", "Coordinator", true);
        ...
        while (running) {
            ...
            switch (choice) {
                ...
                case "3":
                    clearJavaConsoleScreen();
                    while(searchRunning){
                        System.out.println("Please enter the Event ID you want to search for:");
                        String eventId = scanner.nextLine();  // Read user input
                        searchEvent(eventId);
                        System.out.print("Do you want to search another Event? (yes/no): ");
                ...
                case "6":
                    ...
                    while(searchVolunteerRunning){
                        System.out.println("Please enter the Volunteer ID you want to search for:");
                        String volunteerId = scanner.nextLine();  // Read user input
                        ListEventForVolunteer(volunteerId);
                        System.out.print("Do you want to search another Volunteer? (yes/no): ");
                        String response = scanner.nextLine();
                        searchVolunteerRunning = response.equalsIgnoreCase("yes");
                    }
                    break;
                case "7":
                    clearJavaConsoleScreen();
    ...
    public void createEvent() {
        ...
        while (continueCreating) {
            ...
            // Create the event
            Event event = new Event(eventId, name, description, startTime, endTime, location, volunteerList, isActive);
            // Enqueue the event to the linked queue
            eventQueue.enqueue(event);

            System.out.println("Event created and added to the queue successfully!\n");
        
        

            System.out.print("Do you want to create another event? (yes/no): ");
            String response = scanner.nextLine();
    ...
    public void removeEvent() {
        if (eventQueue.isEmpty()) {
            ...
            System.out.println(removedEvent); // Print the removed event's details
        }
    }
    ...
    public void searchEvent(String keyword) {
        ...
        while (iterator.hasNext()) {
            ...
            // Check if the keyword matches the event ID or event name (case insensitive)
            if (event.getEventId().equalsIgnoreCase(keyword) || event.getEventTitle().equalsIgnoreCase(keyword)) {
                eventFound = true;
                System.out.println("Event ID:        " + event.getEventId());
                System.out.println("Event Name:      " + event.getEventTitle());
                System.out.println("Description:     " + event.getDescription());
                System.out.println("Start Time:      " + event.getStartTime().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm")));
                System.out.println("End Time:        " + event.getEndTime().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm")));
                System.out.println("Location:        " + event.getLocation());
                System.out.println("Status:          " + (event.isEventActive() ? "Active" : "Inactive"));
                System.out.println("----------------------------------------");
            }
        }
    ...
    public void amendEvent() {
        ...
        System.out.print("Enter the Event ID of the event you want to amend: ");
        String eventId = scanner.nextLine();

        LinkedQueue<Event> tempQueue = new LinkedQueue<>(); // Temporary queue to hold events
        ...
        if (eventToAmend != null) {
            System.out.println("Amending event: " + eventToAmend.getEventTitle());
            ...
            System.out.print("New Start Time (yyyy-MM-dd HH:mm) (leave blank to keep current): ");
            String newStartTime = scanner.nextLine();
            if (!newStartTime.isBlank()) {
                eventToAmend.setStartTime(LocalDateTime.parse(newStartTime, DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm")));
            }

            System.out.print("New End Time (yyyy-MM-dd HH:mm) (leave blank to keep current): ");
            String newEndTime = scanner.nextLine();
            if (!newEndTime.isBlank()) {
                eventToAmend.setEndTime(LocalDateTime.parse(newEndTime, DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm")));
            }
            ...
            System.out.print("Is the event active? (true/false, leave blank to keep current): ");
    ...
    public void EventsLists() {
        ...
        System.out.printf("%-10s %-30s %-50s %-20s %-20s %-30s %-10s%n",
                        "Event ID", "Event Name", "Description", "Start Time", "End Time", "Location", "Status");
        System.out.println("-------------------------------------------------------------------------------------------------------------");
        ...
        while (iterator.hasNext()) {
            ...
            for (volunteer volunteer : volunteers) {
                if (volunteer.getId().equals(volunteerId)) {
                    ...
                    System.out.println("Status:          " + (event.isEventActive() ? "Active" : "Inactive"));
                    System.out.println("----------------------------------------");
                    break; // Volunteer found, no need to check further in this event
                }
            }
        }

        if (!volunteerFound) {
            System.out.println("No events found for Volunteer ID: " + volunteerId);
        } else {
            System.out.println("End of report for Volunteer ID: " + volunteerId);
        }
    }
    
    public void RemoveVolunteerForEvent(){
        System.out.println("Please enter the Event ID from which the volunteer should be removed:");
        String eventId = scanner.nextLine();
        ...
        System.out.println("Please enter the Volunteer ID to be removed from the event:");
        String volunteerId = scanner.nextLine();

        List<volunteer> volunteers = event.getVolunteers();
        boolean volunteerRemoved = false;

        // Iterate through the list of volunteers and remove the specified volunteer
        ...
        if (volunteerRemoved) {
            System.out.println("Volunteer ID " + volunteerId + " has been removed from Event ID " + eventId + ".");
        } else {
            System.out.println("Volunteer ID " + volunteerId + " not found in Event ID " + eventId + ".");
        }
    }
